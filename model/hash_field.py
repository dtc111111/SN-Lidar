...
class HashGrid4D(nn.Module):
    ...
    def forward_dynamic(self, x, t):
        ...
        assert torch.isfinite(xy).all(), "NaN detected in xy!"
        assert torch.isfinite(xz).all(), "NaN detected in xz!"
        assert torch.isfinite(yz).all(), "NaN detected in yz!"
        ...
        assert torch.isfinite(hash_feat_xyt).all(), "NaN detected in hash_feat_xyt!"
        assert torch.isfinite(hash_feat_xzt).all(), "NaN detected in hash_feat_xzt!"
        assert torch.isfinite(hash_feat_yzt).all(), "NaN detected in hash_feat_yzt!"

        reduction = reduction_func(self.reduction)
        hash_feat_dynamic = reduction([hash_feat_xyt, hash_feat_xzt, hash_feat_yzt])
        assert torch.isfinite(hash_feat_dynamic).all(), "NaN detected in hash_feat_dynamic!"
    ...
    def forward(self, x, t):
        ...
        assert torch.isfinite(t).all(), "t contains NaN or Inf!"
        
        assert ((x >= 0) & (x <= 1)).all(), "x is out of range [0,1]!"
        assert (t >= 0) & (t <= 1), "t is out of range [0,1]!"
        hash_feat_static = self.forward_static(x)
        assert torch.isfinite(hash_feat_static).all(), "NaN detected in hash_static output!"
