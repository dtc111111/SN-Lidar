...
def lidar_to_pano_with_intensities(
    ...
    local_point_labels = local_points_with_intensities[:, 4]
    fov_up, fov = lidar_K
    fov_down = fov - fov_up
    ...
    labels = np.zeros((lidar_H, lidar_W))

    count = 0
    for local_points, dist, local_point_intensity, local_point_label in zip(
        local_points,
        dists,
        ...
        if pano[r, c] == 0.0:
            ...
            labels[r, c] = local_point_label
        elif pano[r, c] > dist:
            pano[r, c] = dist
            intensities[r, c] = local_point_intensity
            labels[r, c] = local_point_label
    return pano, intensities, labels
...
def pano_to_lidar_with_labels(pano, label, lidar_K):
    """
    Args:
        pano: (H, W), float32.
        label: (H, W), float32.
    ...
    Return:
        local_points: (N, 4), float32, in lidar frame.
    """
    fov_up, fov = lidar_K
    ...
    local_points_with_labels = np.concatenate(
        [local_points, label.reshape(H, W, 1)], axis=2
    )

    # Filter empty points.
    idx = np.where(pano != 0.0)
    local_points_with_labels = local_points_with_labels[idx]
    return local_points_with_labels # [H, W, 4]