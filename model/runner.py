...
from utils.convert import pano_to_lidar, pano_to_lidar_with_labels
from utils.misc import point_removal

from data.semantickitti_dataset import LEARNING_MAP, LEARNING_MAP_INV, COLOR_MAP
...
class Trainer(object):
    def __init__(
        ...
        if self.opt.dataloader == "semantickitti":
            self.learning_map = LEARNING_MAP
            self.learning_map_inv = LEARNING_MAP_INV
            self.color_map = COLOR_MAP
        elif self.opt.dataloader == "kitti360":
            pass
    ...
    def map_labels(labels, learning_map):
        mapped_labels = torch.zeros_like(labels, dtype=torch.long)
        for old_label, new_label in learning_map.items():
            mapped_labels[labels == old_label] = new_label
        return mapped_labels
    
    @staticmethod
    def colormap_to_colors(colormap: dict[str, collections.abc.Iterable[int]], name2idx: dict[str, int]) -> np.ndarray:
        colors = []
    ...
    def train_step(self, data):
        ...
        images_depth = data["images_depth"]  # [NV, H, W, 3]
        rays_index = data["rays_index"]  # [B, num_rays]

        gt_raydrop = images_lidar[:, :, 0]
        gt_intensity = images_lidar[:, :, 1] * gt_raydrop
        gt_depth = images_lidar[:, :, 2] * gt_raydrop
        gt_semantic = images_lidar[:, :, 3] * gt_raydrop
        gt_semantic = self.map_labels(gt_semantic, self.learning_map)
        
        self.model.encoder_init(images_depth.permute(0, 3, 1, 2))
        ...
        pred_semantic = outputs_lidar["image_lidar"][:, :, 2:] * gt_raydrop.unsqueeze(-1)
        ...
        depth_loss = self.criterion["depth"](pred_depth, gt_depth)
        raydrop_loss = self.criterion["raydrop"](pred_raydrop, gt_raydrop_smooth)
        intensity_loss = self.criterion["intensity"](pred_intensity, gt_intensity)
        
        lidar_loss = (
            self.opt.alpha_d * depth_loss
            + self.opt.alpha_r * raydrop_loss
            + self.opt.alpha_i * intensity_loss
            + self.opt.alpha_s * self.criterion["semantic"](pred_semantic.permute(0, 2, 1), gt_semantic.long())
        )
        ...
        loss = loss +  self.opt.alpha_cd * chamfer_loss
        ...
        if self.opt.flow_loss:
            ...
            for step in [1, 2]:
                if f"{frame_idx+step}" in self.pc_list.keys():
                    ...
                    loss = loss + self.opt.alpha_cd * chamfer_dist
    ...
    def eval_step(self, data):
        ...
        rays_o_lidar = data["rays_o_lidar"]
        rays_d_lidar = data["rays_d_lidar"]
        rays_index = data["rays_index"]
        ...
        gt_depth = images_lidar[:, :, :, 2] * gt_raydrop
        gt_semantic = images_lidar[:, :, :, 3] * gt_raydrop
        gt_semantic_mapped = self.map_labels(gt_semantic, self.learning_map)

        # first forward without encoder
        outputs_lidar = self.model.render(
            rays_o_lidar,
            rays_d_lidar,
            rays_index,
        ...
        rendered_depth = outputs_lidar["depth_lidar"].unsqueeze(0).unsqueeze(0)
        self.model.encoder_init(rendered_depth)
        # second forward with encoder
        outputs_lidar = self.model.render(
            rays_o_lidar,
            rays_d_lidar,
            rays_index,
        ...
        pred_semantic = pred_rgb_lidar[:, :, :, 2:] # [B, H, W, num_classes]
        ...
        lidar_loss = (
            ...
            + self.opt.alpha_s * self.criterion["semantic"]((pred_semantic * raydrop_mask.unsqueeze(-1)).permute(0,3,1,2), gt_semantic_mapped.long()).mean()
        )
    ...
    def test_step(self, data, perturb=False):
        ...
        pred_raydrop = pred_rgb_lidar[:, :, :, 0]
        pred_intensity = pred_rgb_lidar[:, :, :, 1]
        pred_semantic = pred_rgb_lidar[:, :, :, 2:]
        pred_depth = outputs_lidar["depth_lidar"].reshape(-1, H_lidar, W_lidar)
        if self.opt.raydrop_loss == 'bce':
            pred_raydrop = F.sigmoid(pred_raydrop)
        if self.use_refine:
        ...
        if self.opt.alpha_r > 0:
            ...
            pred_semantic = pred_semantic * raydrop_mask.unsqueeze(-1)

        return pred_raydrop, pred_intensity, pred_depth, pred_semantic
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        with torch.no_grad():
            ...
            for idx, data in enumerate(loader):
                ...
                preds_semantic = preds_semantic[0].detach()
                preds_semantic = torch.argmax(preds_semantic, dim=2)
                gt_semantic_mapped = self.map_labels(gt_semantic, self.learning_map)
                ...
                for i, metric in enumerate(self.lidar_metrics):
                    if i == 0:  # hard code
                        ...
                        depth_mask = metric.update(preds_depth*preds_mask, gt_depth) # DepthMeter
                    elif i == 3:
                        metric.update(preds_depth*preds_mask, gt_depth) # PointMeter
                    elif i == 4:
                        metric.update(preds_semantic*preds_mask, gt_semantic_mapped, depth_mask) # SegmentationMeter
                ...
                save_path_pred_ply = os.path.join(
                    self.workspace,
                    "validation",
                    "ply",
                    f"{name}_{self.local_step:04d}.ply",
                )
                os.makedirs(os.path.dirname(save_path_pred_ply), exist_ok=True)

                # save images
                gt_raydrop = gt_raydrop[0].detach().cpu().numpy()
                img_gt_raydrop = (gt_raydrop * 255).astype(np.uint8)
                img_gt_raydrop = cv2.cvtColor(img_gt_raydrop, cv2.COLOR_GRAY2BGR)

                gt_intensity = gt_intensity[0].detach().cpu().numpy()
                img_gt_intensity = (gt_intensity * 255).astype(np.uint8)
                img_gt_intensity = cv2.applyColorMap(img_gt_intensity, 1)

                gt_depth = gt_depth[0].detach().cpu().numpy()
                img_gt_depth = (gt_depth * 255).astype(np.uint8)
                img_gt_depth = cv2.applyColorMap(img_gt_depth, 20)
                
                gt_semantic = gt_semantic[0].detach().cpu().numpy()
                img_gt_semantic = np.zeros((gt_semantic.shape[0], gt_semantic.shape[1], 3), dtype=np.uint8)
                for label, color in self.color_map.items():
                    img_gt_semantic[gt_semantic == label] = color
                ...
                preds_semantic = preds_semantic.detach().cpu().numpy()
                img_semantic = np.zeros((preds_semantic.shape[0], preds_semantic.shape[1], 3), dtype=np.uint8)
                inv_map_array = np.array([self.learning_map_inv[i] for i in range(self.opt.num_classes)])
                preds_semantic_inv = inv_map_array[preds_semantic]
                for label, color in self.color_map.items():
                    img_semantic[preds_semantic_inv == label] = color
                img_semantic = img_semantic.astype(np.uint8)
                ...
                img_semantic_masked = img_semantic * np.expand_dims(preds_mask, axis=-1)
                img_semantic_masked = img_semantic_masked.astype(np.uint8)
                
                img_pred = cv2.vconcat([
                    img_gt_raydrop, img_gt_intensity, img_gt_depth, img_gt_semantic,
                    img_raydrop, img_intensity, img_depth, img_semantic,
                    img_raydrop_masked, img_intensity_masked, img_depth_masked, img_semantic_masked
                ])
                
                cv2.imwrite(save_path_pred, img_pred)
                
                # save point clouds
                pred_lidar_label = pano_to_lidar_with_labels(
                    pred_depth * preds_mask / self.opt.scale, preds_semantic_inv, loader._data.intrinsics_lidar
                )
                
                pred_ply = o3d.geometry.PointCloud()
                pred_ply.points = o3d.utility.Vector3dVector(pred_lidar_label[:, :3])
                pred_color = np.zeros_like(pred_lidar_label[:, :3])
                for label, c in self.color_map.items():
                    pred_color[pred_lidar_label[:, -1] == label] = c[2], c[1], c[0] # BGR to RGB
                pred_ply.colors = o3d.utility.Vector3dVector(pred_color / 255)
                ...
                o3d.io.write_point_cloud(
                    os.path.join(
                        ...
                        "validation",
                        "ply",
                        f"{name}_{self.local_step:04d}_lidar.ply",
                    ),
                    pred_ply,
                )
        ...
        if len(self.lidar_metrics) > 0:
            result = self.lidar_metrics[-1].measure()[0]  # hard code, point meter
    ...
    def test(self, loader, save_path=None, name=None, write_video=True, refine=True):
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds_raydrop, preds_intensity, preds_depth, pred_semantic = self.test_step(data)
                ...
                pred_semantic = pred_semantic[0].detach().cpu().numpy()
                pred_semantic = np.argmax(pred_semantic, axis=2)
                img_semantic = np.zeros((pred_semantic.shape[0], pred_semantic.shape[1], 3), dtype=np.uint8)
                inv_map_array = np.array([self.learning_map_inv[i] for i in range(20)])
                pred_semantic = inv_map_array[pred_semantic]
                for label, color in self.color_map.items():
                    img_semantic[pred_semantic == label] = color[2], color[1], color[0]
                img_semantic = img_semantic.astype(np.uint8)
                
                # save ply
                pred_lidar_label = pano_to_lidar_with_labels(
                    pred_depth / self.opt.scale, pred_semantic, loader._data.intrinsics_lidar
                )
                pred_label_ply = o3d.geometry.PointCloud()
                pred_label_ply.points = o3d.utility.Vector3dVector(pred_lidar_label[:, :3])
                pred_label_color = np.zeros_like(pred_lidar_label[:, :3])
                for label, c in self.color_map.items():
                    pred_label_color[pred_lidar_label[:, -1] == label] = c[2], c[1], c[0] # BGR 2 RGB
                pred_label_ply.colors = o3d.utility.Vector3dVector(pred_label_color / 255)

                if write_video:
                    all_preds_intensity.append(cv2.cvtColor(cv2.applyColorMap(pred_intensity, 1), cv2.COLOR_BGR2RGB))
                    all_preds_depth.append(cv2.cvtColor(cv2.applyColorMap(pred_depth, 20), cv2.COLOR_BGR2RGB))
                    all_preds_semantic.append(img_semantic)
                else:
                    os.makedirs(os.path.join(save_path, "raydrop"), exist_ok=True)
                    cv2.imwrite(
                        os.path.join(save_path,"raydrop", f"test_{name}_{i+1:04d}_raydrop.png"),
                        pred_raydrop,
                    )
                    os.makedirs(os.path.join(save_path, "intensity"), exist_ok=True)
                    cv2.imwrite(
                        os.path.join(
                            save_path, "intensity", f"test_{name}_{i+1:04d}_intensity.png"
                        ),
                        cv2.applyColorMap(pred_intensity, 1),
                    )
                    os.makedirs(os.path.join(save_path, "depth"), exist_ok=True)
                    cv2.imwrite(
                        os.path.join(save_path, "depth", f"test_{name}_{i+1:04d}_depth.png"),
                        cv2.applyColorMap(pred_depth, 20),
                    )
                    os.makedirs(os.path.join(save_path, "semantic"), exist_ok=True)
                    cv2.imwrite(
                        os.path.join(save_path, "semantic", f"test_{name}_{i+1:04d}_semantic.png"),
                        img_semantic,
                    )
                
                save_path_pred_ply = os.path.join(save_path, "ply", f"test_{name}_{i+1:04d}_lidar_semantic.ply")
                os.makedirs(os.path.dirname(save_path_pred_ply), exist_ok=True)
                o3d.io.write_point_cloud(save_path_pred_ply, pred_label_ply)

                pbar.update(loader.batch_size)

        if write_video:
            all_preds_intensity = np.stack(all_preds_intensity, axis=0)
            all_preds_depth = np.stack(all_preds_depth, axis=0)
            all_preds_semantic = np.stack(all_preds_semantic, axis=0)
            imageio.mimwrite(
                os.path.join(save_path, f"{name}_intensity.mp4"),
                all_preds_intensity,
                fps=25,
                quality=8,
                macro_block_size=1,
            ...
            imageio.mimwrite(
                os.path.join(save_path, f"{name}_semantic.mp4"),
                all_preds_semantic,
                fps=25,
                quality=8,
                macro_block_size=1,
    ...
    def refine(self, loader):
        ...
        raydrop_input = torch.cat(raydrop_input_list, dim=0).cuda().float().contiguous()
        raydrop_gt = torch.cat(raydrop_gt_list, dim=0).cuda().float().contiguous()
        ...
        for i in range(refine_epoch):
            ...
            raydrop_refine = self.model.unet(input) # unet output: refined pred raydrop
