...
from utils.convert import pano_to_lidar_with_intensities, pano_to_lidar_with_labels
from utils.semantic_helpers import get_semantic_img, get_semantic_ply, get_ply
...
class Simulator(object):
    ...
    @torch.no_grad()
    def render(self, rays_o_lidar, rays_d_lidar, inds, times_lidar, sequence_id, save_pc=True, save_img=True, save_video=True):
        if save_video:
            all_preds = []
            all_preds_depth = []
            all_preds_intensity = []
        ...
        for i in tqdm.tqdm(range(B)):
            ...
            pred_semantic = pred_semantic * raydrop_mask
            ...
            pred_lidar_semantic = pano_to_lidar_with_labels(
                pred_depth / self.opt.scale, pred_semantic, self.opt.fov_lidar
            )
            ...
            if save_pc:
                ...
                os.makedirs(os.path.dirname(save_path_label), exist_ok=True)
                ply_labels = get_semantic_ply(pred_lidar_semantic, self.opt.dataloader)
                o3d.io.write_point_cloud(save_path_label, ply_labels)

            if save_img:
                save_path = os.path.join(
                        self.workspace,
                        "images",
                        f"snlidar_{i:04d}.png",
                    )
                ...
                img_semantic = get_semantic_img(pred_semantic, self.opt.dataloader)
                
                img_pred = cv2.vconcat([img_raydrop, img_intensity, img_depth, img_semantic])
